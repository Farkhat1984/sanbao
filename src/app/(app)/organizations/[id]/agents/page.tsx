...
import { ArrowLeft, Plus, Robot, FileText, SpinnerGap, CheckCircle, WarningCircle } from "@phosphor-icons/react";
...
const STATUS_CONFIG: Record<string, { label: string; icon: React.ElementType; color: string }> = {
  CREATING: { label: "Создание", icon: SpinnerGap, color: "text-accent" },
  PROCESSING: { label: "Обработка", icon: SpinnerGap, color: "text-warning" },
  READY: { label: "Готов", icon: CheckCircle, color: "text-success" },
  PUBLISHED: { label: "Опубликован", icon: CheckCircle, color: "text-accent" },
  ERROR: { label: "Ошибка", icon: WarningCircle, color: "text-error" },
};
                  ...
                  >
                    <div className="flex items-start gap-3 mb-3">
                      <div className="h-10 w-10 rounded-xl bg-accent/10 flex items-center justify-center shrink-0">
                        <Robot weight="duotone" className="h-5 w-5 text-accent" />
                      </div>
                    ...
                    <div className="flex items-center gap-4 text-xs text-text-secondary">
                      <span className={cn("flex items-center gap-1", config.color)}>
                        <StatusIcon weight={agent.status === "CREATING" || agent.status === "PROCESSING" ? "bold" : "duotone"} className={cn("h-3.5 w-3.5", agent.status === "PROCESSING" && "animate-spin")} />
                        {config.label}
                      </span>
        ...
        {!isLoading && agents.length === 0 && (
          <div className="text-center py-20">
            <div className="h-16 w-16 rounded-2xl bg-surface-alt flex items-center justify-center mx-auto mb-5">
              <Robot weight="duotone" className="h-8 w-8 text-text-secondary" />
            </div>
